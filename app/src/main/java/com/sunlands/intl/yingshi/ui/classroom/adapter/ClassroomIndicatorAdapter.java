 ...
 * 创建时间: 2018/8/6
 */
public class ClassroomIndicatorAdapter extends CommonNavigatorAdapter {
    private final List<String> mDataList;
    Context mContext;
    ViewPager mViewPager;

    public ClassroomIndicatorAdapter(Context context, ViewPager viewPager, int arrayResId) {
        mContext = context;
    ...
    public IPagerTitleView getTitleView(Context context, final int index) {
        ...
        simplePagerTitleView.setPadding(UIUtil.dip2px(mContext, 5), 0,
                UIUtil.dip2px(mContext, 5), 0);
        simplePagerTitleView.setTextSize(TypedValue.COMPLEX_UNIT_SP, 16);
        simplePagerTitleView.setText(mDataList.get(index));
        simplePagerTitleView.setNormalColor(mContext.getResources().getColor(R.color.text999));
