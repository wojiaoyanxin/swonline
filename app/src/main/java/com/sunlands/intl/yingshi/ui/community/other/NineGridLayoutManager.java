...
public class NineGridLayoutManager extends RecyclerView.LayoutManager {
    ...
    public NineGridLayoutManager(Context context, int Width, int Height) {
        DisplayMetrics dm = context.getResources().getDisplayMetrics();

        gridSpacing = (int) TypedValue.applyDimension(TypedValue.COMPLEX_UNIT_DIP, 3, dm);
        singleImageWidth = (int) TypedValue.applyDimension(TypedValue.COMPLEX_UNIT_DIP, Width, dm);
        singleImageHeight = (int) TypedValue.applyDimension(TypedValue.COMPLEX_UNIT_DIP, Height, dm);
        mCacheBorders = new Pool<>(new Pool.New<Rect>() {
            @Override
            public Rect get() {
                return new Rect();
